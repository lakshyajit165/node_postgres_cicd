...
describe("---------------------- Testing Task APIs -------------------------", () => {
    ...
    describe("/GET task by an id not present in db", () => {
        it("it SHOULD RETURN appropriate error message when no task with an id is found", (done) => {
            chai.request(app)
                .get("/get_task/123456")
                .end((err, res) => {
                    res.should.have.status(404);
                    res.body.should.have.property("errorMessage");
                    res.body.errorMessage.should.equal("No task found with the given id!");
                    done();
                });
        });
    });
